...
class EbookProfile extends StatefulWidget {
  final User user; // User object passed from the previous screen

  const EbookProfile({super.key, required this.user});

  @override
  _EbookProfileState createState() => _EbookProfileState();
}

class _EbookProfileState extends State<EbookProfile> {
  bool isLoading = true;
  String errorMessage = '';
  List<Map<String, dynamic>> ebookPackages = [];
  ...
  /// Fetches the eBook package details from the API.
  Future<void> _fetchEbookPackages() async {
    try {
      ...
      if (response.statusCode == 200) {
        ...
        if (data['services'] != null && data['services'] is List<dynamic>) {
          ...
          // Filter services where category_name is 'ebooks' and package_status is 'Available'
          final ebooks = services.where((service) {
            final categoryName =
                service['category_name']?.toString().toLowerCase();
            final packageStatus =
                service['package_status']?.toString().toLowerCase();
            return categoryName == 'ebooks' && packageStatus == 'available';
          }).toList();

          if (ebooks.isNotEmpty) {
            setState(() {
              ebookPackages =
                  ebooks.map((ebook) => ebook as Map<String, dynamic>).toList();
              isLoading = false;
            });
          } else {
            setState(() {
              errorMessage = 'No available eBook package found for this user.';
              isLoading = false;
            });
          }
        } else {
          setState(() {
            errorMessage = 'E-Book services are currently not available.';
            isLoading = false;
          });
        }
      } else {
      ...
      print('Exception in fetching ebook packages: $e');
      setState(() {
                    ...
                    child: Column(
                      children: [
                        ...
                        ListView.builder(
                          ...
                          itemCount: ebookPackages.length,
                          itemBuilder: (context, index) {
                            final package = ebookPackages[index];
